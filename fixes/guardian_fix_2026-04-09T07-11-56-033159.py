...
Description: Add early validation check for empty corpus (n_docs=0 or vocab_size=0) before calling tm_lda.evaluate_topic_models. When empty, return a graceful result with error info instead of crashing with scipy ValueError.
Error Type: ValueError
Timestamp: 2026-04-09T07:11:56.032629
"""

from __future__ import annotations

"""Fix for topic_modeler — handles empty corpus gracefully.

Root Cause:
    The paper_fetcher returned 0 papers, so the preprocessor produced an empty
    corpus (n_docs=0, vocab_size=0). When tm_lda.evaluate_topic_models receives
    a zero-sized DTM, scipy.sparse.coo_matrix raises:
        ValueError: cannot infer dimensions from zero sized index arrays

Fix:
    Add an early validation check after loading the DTM. If the corpus is empty
    (0 documents or 0 vocabulary), skip topic modeling and return a graceful
...
class TopicModeler(BaseModule):
    ...
    def process(self, input_data: dict, config: dict, context: RunContext) -> dict:
        ...
        # --- FIX: Early validation for empty corpus ---
        if n_docs == 0 or vocab_size == 0:
            logger.warning(
                "Empty corpus detected (n_docs=%d, vocab_size=%d). "
                "Skipping topic modeling. This usually means the paper_fetcher "
                "returned no papers or the preprocessor filtered out all documents.",
                n_docs, vocab_size,
            )
            # Create a placeholder output so downstream modules don't crash
            return {
                "model_path": None,
                ...
                "stats": {
                    ...
                    "error": "empty_corpus",
                    "message": (
                        f"No documents available for topic modeling "
                        f"(n_docs={n_docs}, vocab_size={vocab_size}). "
                        "Check the paper_fetcher and preprocessor outputs."
                    ),
                },
            }
        ...
        topic_word_df = ldamodel_top_topic_words(
            best_model, vocab, top_n=top_n_words, return_type="pandas"
        )
        topic_word_path = output_dir / "topic_words.csv"
        topic_word_df.to_csv(topic_word_path, index=True)

        doc_topic_df = ldamodel_top_topic_docs(
            best_model, doc_labels, top_n=top_n_words, return_type="pandas"
        )
        ...
        topic_labels = generate_topic_labels_from_top_words(
            best_model, vocab, doc_lengths(dtm_matrix),
            top_n=top_n_words, lambda_=lambda_,
        )
        ...
        vis_params = parameters_for_ldavis(
            best_model, dtm_matrix, doc_lengths(dtm_matrix),
            vocab, R=30, lambda_step=0.01,
        )
    ...
    def _select_best_k(self, eval_by_k, metric, min_k, max_k):
        ...
        For coherence_mimno_2011: higher is better.
        For arun_2010 and cao_juan_2009: lower is better (they measure perplexity-like quantities).
        """
        import numpy as np

        ks = [r[0] for r in eval_by_k]
        values = [r[2].get(metric, float("nan")) for r in eval_by_k]

        # Filter out NaN values
        valid = [(k, v) for k, v in zip(ks, values) if not np.isnan(v)]
        if not valid:
            logger.warning("No valid %s values found, using default K=%d", metric, min_k)
            return min_k

        valid_ks = [v[0] for v in valid]
        valid_values = [v[1] for v in valid]

        # Coherence: higher is better; Arun/CaoJuan: lower is better
        coherence_metrics = {"coherence_mimno_2011", "coherence_uchida_2017", "held_out_likelihood"}
        if metric in coherence_metrics:
            best_idx = int(np.argmax(valid_values))
        else:
            best_idx = int(np.argmin(valid_values))

        return valid_ks[best_idx]
