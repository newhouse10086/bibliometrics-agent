...
Fix:
    ...
    Also add bounds checking for max_k relative to n_docs and n_vocab.
"""
...
class TopicModeler(BaseModule):
    ...
    def process(self, input_data: dict, config: dict, context: RunContext) -> dict:
        ...
        if max_k >= n_vocab:
            ...
            logger.warning("Reduced max_topics to %d (must be < n_vocab=%d)", max_k, n_vocab)
        ...
        doc_topic_df = ldamodel_top_topic_docs(best_model, doc_labels, top_n=top_n_words)
        ...
        # --- Step 6: Generate pyLDAvis visualization ---
        ...
        topic_words_dict = {
            f"Topic_{i}": list(row) for i, row in enumerate(topic_word_df.values)
        }
        ...
        doc_lens = doc_lengths(dtm_matrix)
        stats = {
            "n_topics": best_k,
            "selected_k": best_k,
            "coherence": float(best_result[1][selection_metric]),
            "n_docs": int(n_docs),
            "vocab_size": int(n_vocab),
            "avg_doc_length": float(np.mean(doc_lens)),
        }

        logger.info("Topic modeling complete. Stats: %s", stats)
    ...
    def _select_best_k(self, eval_by_k, metric, min_k, max_k):
        ...
        For coherence metrics (higher is better), pick the K with max value.
        For Arun/CaoJuan (lower is better), pick the K with min value.
        """
        values = [(r[0], r[1][metric]) for r in eval_by_k if metric in r[1]]

        if not values:
            logger.warning("No valid results for metric %s, falling back to K=%d", metric, min_k)
            return min_k

        # Coherence metrics: higher is better
        coherence_metrics = ["coherence_mimno_2011", "coherence_uci_2007", "coherence_umass_2010"]
        if metric in coherence_metrics:
            best = max(values, key=lambda x: x[1])
        else:
            # Arun, CaoJuan: lower is better
